...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    if( t<maxd ) {
        ...
        // Pure NeuroSphere violet-blue color scheme
        col = vec3(0.1,0.05,0.2); // Deep violet base
        col += 0.2*tra;

        // Pure violet-blue patterns only (no green, yellow, red)
        vec3 pat = vec3(
            0.4 + 0.3 * sin(iTime * 0.8 + pos.x * 2.0), // Violet-red component
            0.2 + 0.2 * sin(iTime * 1.2 + pos.y * 4.0 + 2.0), // Minimal green (for blue)
            0.9 + 0.1 * sin(iTime * 1.6 + pos.z * 6.0 + 4.0) // Strong blue component
        );

        // Force pure violet-blue colors
        pat = vec3(pat.r, pat.g * 0.3, pat.b); // Suppress green, enhance blue
        ...
        // Pure violet-blue lighting only
        lin += 3.5*dif*vec3(0.6,0.1,2.0)*pow(vec3(sha),vec3(1.0,1.2,1.5));
		lin += 1.0*amb*vec3(0.3,0.05,0.6)*occ;
		lin += 1.0*bac*vec3(0.8,0.1,1.8)*occ;
		lin += 1.0*bou*vec3(0.5,0.05,1.5)*occ;
        lin += 4.0*fre*vec3(0.9,0.2,1.8)*(0.3+0.7*dif*sha)*occ;
        lin += spe*2.0;
    ...
    // Force pure violet-blue tones only
    col = pow( col, vec3(0.9,0.3,0.7) );
